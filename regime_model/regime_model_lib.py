...
def Q_Q_plot(asset_data, column_num, ret, ret_g, ret_c):
...
def regime_plot(asset_data, column_num, ret, ret_g, ret_c):
...
def trend_filtering(data, lambda_value):
    ...
    return beta.value as the solution to the optimization problem
    '''
    n = np.size(data)
...
def efficient_frontier_traditional(r_annual, Q_all, r_bar):
    ...
    # for each target return, solve an optimiztaion problem
...
def regime_asset(n,mu1,mu2,Q1,Q2,p1,p2):
    ...
    regime = np.ones(n)
    for i in range(n-1):
        # if we are currently in the normal regime
        if regime[i] == 1:
            if np.random.rand() > p1:
                # switch to crash regime
                regime[i+1] = 0
        # if we are currently in the crash regime
        else:
            if np.random.rand() > p2:
                # stay in the crash regime
                regime[i+1] = 0
...
def asset_simulation(assets_info, asset_num, regime_name, 
    ...
    assets_info is a pandas Dataframe containing asset total return indices; 
        please refer to the dataset for format.
    asset_num is the number of assets we would like to use. 
        By default, this should be the first few columns in dataset.
...
def fund_simulation(holdings, asset_return, hold_type='fixed',spending_rate=0.03):
    ...
    The simulation returns a full path of wealth at the end of each year, 
        so it is a n_scenarios*n_years matrix.
    """
